...
public abstract class Node{
	...
	public int findKeyIndex (int val){
		...
		for(;i<=lastindex;i++){
			...
			//if val>keys[lastIndex] return lastIndex
	...
	public int findPtrIndex (int val){
		...
		for(;index<=lastindex;index++){
			if(index+1<=lastindex&&keys[index+1]>val) 
				break;
			//if val>keys[lastIndex] return lastIndex
	...
	public void insertLeftDef (int val,Node ptr,int i) {
		//shift
		int temp = lastindex;
		while(temp>=i){
		...
		ptrs[temp]=ptr;
		ptr.setParent(new Reference(this, temp, false));
		lastindex++;
	}
	
	
	public void shiftLeftDef (int i) {
		while(i<lastindex){
			keys[i]=keys[i+1];
			ptrs[i-1]=ptrs[i];
			ptrs[i-1].getParent().decreaseIndex();
			i++;
		}
		ptrs[i-1]=ptrs[i];
		ptrs[i-1].getParent().decreaseIndex();
	...
	public abstract void combineLeft();
	...
	public abstract void redistribute(Node sibling);
